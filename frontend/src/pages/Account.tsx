...
export const Account = (): JSX.Element => {
	...
	return (
		<>
			<SectionHero title='Account Settings' subTitle='Manage your profile, security, and account preferences.' />
							...
							{isEditingProfile ? (
								<div className='space-y-4'>
									<div className='flex flex-wrap items-center gap-4'>
										<div className='h-16 w-16 overflow-hidden rounded-full bg-orange-100'>{user?.imageUrl ? <img src={user.imageUrl} alt='Profile' className='h-full w-full object-cover' /> : <div className='flex h-full w-full items-center justify-center text-sm font-semibold text-orange-700'>{user?.firstName?.[0] || user?.lastName?.[0] || 'U'}</div>}</div>
										<div className='space-y-1'>
											<p className='text-xs font-semibold uppercase tracking-widest text-text-subtle'>Profile Photo</p>
								...
								<div className='space-y-3'>
									<div className='flex items-center gap-4'>
										...
										<div>
											<p className='text-xs font-semibold uppercase tracking-widest text-text-subtle'>Profile Photo</p>
											<p className='text-sm text-text'>Click edit to update your photo.</p>
										</div>
									</div>
									<div>
								...
								isEditingPreferences ? (
									<div className='space-y-4'>
										<div>
											...
											<Select value={preferencesData.tripStyle} onChange={(e) => setPreferencesData({ ...preferencesData, tripStyle: e.target.value as any })} className='mt-1' options={[]}>
										...
										<div>
											...
											<Select value={preferencesData.budget} onChange={(e) => setPreferencesData({ ...preferencesData, budget: e.target.value as any })} className='mt-1' options={[]}>
						...
						<Card className='space-y-4 p-6'>
							...
							{!showDeleteConfirm ? (
								<div className='space-y-3'>
									...
									<Button variant='outline' onClick={() => setShowDeleteConfirm(true)} fullWidth className='text-white border-red-600 bg-red-600 hover:!border-red-600 hover:!bg-red-700 gap-2'>
								...
								<div className='space-y-4'>
									<div className='bg-red-50 border border-red-200 rounded-lg p-4'>
										<p className='text-sm font-semibold text-red-900 mb-2'>⚠️ Are you absolutely sure?</p>
										<p className='text-xs text-red-700'>This will permanently delete your account, all saved itineraries, and preferences. This action cannot be undone.</p>
									</div>
